function bootstrap_inputs(fields, values) {
  return Object.entries(fields).reduce(
    (a, [k, v]) => `${a}<div class="form-floating mb-3">
...
customElements.define(
  "toast-container",
  class ToastWebcomponent extends HTMLElement {
    connectedCallback() {
      this.classList.add(
        "toast-container",
        "position-fixed",
      ...
      addEventListener("toast-error", (e) =>
        this.showToast({ ...e.detail, style: "text-bg-warning" })
      );
      addEventListener("toast-success", (e) =>
        this.showToast({ ...e.detail, style: "text-bg-success" })
      );
    }
    showToast({
      title,
...
// ListGroup component removed - using custom pharmacy-ods-input and pharmacist-input components instead
// Those components provide more specific functionality with validation and remove buttons
